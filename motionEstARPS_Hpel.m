...
function [motionVect, ARPScomputations] = motionEstARPS_Hpel(imgP, imgI, mbSize, p)
...
for i = 1 : mbSize : row-mbSize+1
    for j = 1 : mbSize : col-mbSize+1
        ...
        %costs(3) = costFunc_hpel(imgP(i:i+mbSize-1,j:j+mbSize-1), imgI(i:i+mbSize-1,j:j+mbSize-1),mbSize);
        [x2,y2] = meshgrid(i:i+mbSize-1,j:j+mbSize-1);
        [x3,y3] = meshgrid(i-1.5:i+mbSize-2,j-1.5:j+mbSize-2);
        % interpolate at 1/2 pel accuracy
        z1 = interp2(x2,y2,imgI(i:i+mbSize-1,j:j+mbSize-1),x3,y3,'*linear');
        Indx = isnan(z1);
        ...
        for k = 1:maxIndex
            ...
            clear dd z1 CurrentBlk x2 y2 x3 y3;
            [x2,y2] = meshgrid(refBlkVer:refBlkVer+mbSize-1, refBlkHor:refBlkHor+mbSize-1);
            [x3,y3] = meshgrid(refBlkVer-1.5:refBlkVer+mbSize-2,refBlkHor-1.5:refBlkHor+mbSize-2);
            % interpolate at 1/2 pel accuracy
            z1 = interp2(x2,y2,imgI(refBlkVer:refBlkVer+mbSize-1, refBlkHor:refBlkHor+mbSize-1),x3,y3,'*linear');
            Indx = isnan(z1);
            CurrentBlk = imgP(i:i+mbSize-1,j:j+mbSize-1);
            z1(Indx == 1) = CurrentBlk(Indx==1);
            dd = CurrentBlk - round(z1);
        ...
        vectors(1,mbCount) = y - i + 0.5;    % row co-ordinate for the vector
        vectors(2,mbCount) = x - j + 0.5;    % col co-ordinate for the vector            
        mbCount = mbCount + 1;
        costs = ones(1,6) * 65537;
